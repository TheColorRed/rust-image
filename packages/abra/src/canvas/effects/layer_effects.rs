use crate::{
  Image,
  canvas::{
    effects::{DropShadow, Stroke, drop_shadow, stroke},
...
impl LayerEffects {
  ...
  pub(crate) fn apply(&self, image: Arc<Image>) -> Arc<Image> {
    ...
    if let Some(stroke_opts) = &self.stroke {
      result_image = stroke::apply_stroke(result_image, stroke_opts);
    }

    if let Some(drop_shadow_opts) = &self.drop_shadow {
      result_image = drop_shadow::apply_drop_shadow(result_image, drop_shadow_opts);
    }
