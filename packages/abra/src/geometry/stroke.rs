...
use crate::geometry::{Area, Path, PointF};
...
impl Path {
  ...
  pub fn stroke(&self, p_width: f32, _p_join: LineJoin, p_cap: LineCap) -> Path {
    ...
    stroke_path.with_move_to(left_points[0]);
    if cfg!(debug_assertions) {
      println!(
        "Stroke: left[0]={:?}, left[1]={:?}, right[0]={:?}, right[1]={:?}",
        left_points[0],
        left_points.get(1),
        right_points[0],
        right_points.get(1)
      ...
      if i < flattened.len() - 1 {
        if let LineJoin::Round = _p_join {
          ...
          for arc_pt in arc_pts.iter().skip(1) {
            stroke_path.with_line_to(*arc_pt);
          }
          continue;
        }
      }
    ...
    match p_cap {
      LineCap::Round => {
        ...
        if cfg!(debug_assertions) {
          println!(
            "end cap pts count={} first={:?} mid={:?} last={:?}",
            cap_pts.len(),
            cap_pts.first(),
            cap_pts.get(cap_pts.len() / 2),
            cap_pts.last()
          );
        }
        if cfg!(debug_assertions) {
          let first = cap_pts.first().copied().unwrap_or(end_left);
          let last = cap_pts.last().copied().unwrap_or(end_right);
          let d1 = (first.x - end_left.x).abs() + (first.y - end_left.y).abs();
          let d2 = (last.x - end_right.x).abs() + (last.y - end_right.y).abs();
          if d1 > 1e-3 || d2 > 1e-3 {
            println!("End cap mismatch: d1={} d2={}", d1, d2);
          }
          let mididx = cap_pts.len() / 2;
          let mid = cap_pts[mididx];
          println!("End cap mid: {:?}, tangent(u)=(x={},y={})", mid, ux, uy);
        }
      ...
      LineCap::Square => {
        ...
        let ux = end_dx / seg_len;
        let uy = end_dy / seg_len;
        stroke_path.with_line_to(PointF::new(end_p2.x + ux * half_width, end_p2.y + uy * half_width));
      }
      LineCap::Butt => {}
    }
    ...
    match p_cap {
      LineCap::Round => {
        ...
        if cfg!(debug_assertions) {
          println!(
            "start cap pts count={} first={:?} mid={:?} last={:?}",
            cap_pts.len(),
            cap_pts.first(),
            cap_pts.get(cap_pts.len() / 2),
            cap_pts.last()
        ...
        if cfg!(debug_assertions) {
          let first = cap_pts.first().copied().unwrap_or(start_right);
          let last = cap_pts.last().copied().unwrap_or(start_left);
          let d1 = (first.x - start_right.x).abs() + (first.y - start_right.y).abs();
          let d2 = (last.x - start_left.x).abs() + (last.y - start_left.y).abs();
          if d1 > 1e-3 || d2 > 1e-3 {
            println!("Start cap mismatch: d1={} d2={}", d1, d2);
          }
          let mididx = cap_pts.len() / 2;
          let mid = cap_pts[mididx];
          println!("Start cap mid: {:?}, outward(u)=(x={},y={})", mid, ux, uy);
        }
        for pt in cap_pts.iter().skip(1) {
          stroke_path.with_line_to(*pt);
        }
      ...
      LineCap::Square => {
        ...
        stroke_path.with_line_to(PointF::new(start_p1.x - ux * half_width, start_p1.y - uy * half_width));
      }
      LineCap::Butt => {}
    }

    stroke_path.with_line_to(left_points[0]);
...
impl Area {
  ...
  pub fn stroke(&self, p_width: f32, _p_join: LineJoin) -> Area {
    ...
    if !outer_points.is_empty() {
      stroke_path.with_move_to(outer_points[0]);
      ...
      stroke_path.with_line_to(outer_points[0]);
    }
