...
pub fn vibrance(
  p_image: &mut Image, p_vibrance: impl Into<f64>, p_saturation: impl Into<f64>, p_options: impl Into<Options>,
) {
  let vibrance = (p_vibrance.into() as f32).clamp(-100.0, 100.0);
  let saturation = (p_saturation.into() as f32).clamp(-100.0, 100.0);

  apply_adjustment!(apply_vibrance, p_image, p_options, 1, vibrance, saturation);
}
