import { CommandId, getCommand, runCommand } from '@/data/commands';
import { MenuItem, menubarItems as menuItems } from '@/data/menu-bar-items';
import { cn } from '@/lib/util';
import { acceleratorDisplayName } from '@/services/accelerator';
...
function TitleBarMenuItem({ item }: { item: MenuItem }) {
  ...
  const handleOnMenuAction = (args?: any[]) => {
    runCommand(item.command ?? ('' as CommandId), ...(args ?? []));
    setOpenSubmenu(null);
    setOpenMenu(null);
  };
  ...
  const accelerator = useMemo(() => {
    const cmd = item.command as CommandId;
    return getCommand(cmd)?.key;
  }, [item]);
    ...
    <div data-label={item.label} className="text-active" ref={divRef}>
      {item.type === 'separator' ? (
        ...
        <div
          ...
          onClick={() => handleOnMenuAction(item.commandArgs)}
          tabIndex={0}
        ...
        >
          ...
          {accelerator && <span className="ml-2 text-gray-400">{acceleratorDisplayName(accelerator)}</span>}
        </div>
      )}
      {item.submenu && (
        <>
          ...
          {isDev && (
            <MenuItemRoot
              menuItem={{
                label: 'Dev',
                submenu: [
                  { label: 'Refresh', command: 'dev-reload-renderer' },
                  { label: 'Toggle DevTools', command: 'dev-toggle-devtools' },
                ],
              }}
              openMenu={openMenu}
              openMenuPosition={openMenuPosition}
