...
import { DragItem, DragListContainer } from '@/ui/drag';
  ...
  const onAddLayer = useCallback(
    async (action: NewLayerAction, options?: any) => {
      ...
      if (action === 'layer-file') {
        const file = (await window.alakazam.openFileDialog('Add Layer From File'))?.[0];
        if (file) {
...
export function LayersPanel() {
  ...
  return (
    <LayersContext.Provider value={{ activeProject, projectMetadata }}>
      <div className="h-full min-h-50 flex-1">
        <Layers />
      </div>
