import { type AddLayerOptions } from '@/server/events/projects';
import { LayerMetadata, ProjectMetadata } from 'bindings';
import { contextBridge, ipcRenderer, OpenDialogOptions } from 'electron';
import { auditTime, Subject } from 'rxjs';

type Layer = import('../../bindings').Layer;
type Project = import('../../bindings').Project;
...
export interface AlakazamApi {
  ...
  openFileDialog: (settings?: OpenDialogOptions['properties']) => Promise<string[] | null>;
  ...
  adjustments: {
     ...
     */
    applyInstantAdjustment: (adjustmentType: 'autoColor' | 'autoTone' | 'invert' | 'grayscale') => Promise<void>;
  };
...
contextBridge.exposeInMainWorld('alakazam', {
  openFileDialog: async settings => ipcRenderer.invoke('open-file-dialog', settings),
