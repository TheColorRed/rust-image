...
pub fn gaussian_blur(layer: &mut Layer, radius: f64) {
  let layer_ref = layer.get_underlying_layer_mut();
  blur::gaussian_blur(&mut *layer_ref, radius as u32, None);
  layer.mark_dirty();
}
#[napi]
...
pub fn surface_blur(layer: &mut Layer, radius: f64, threshold: f64) {
  let layer_ref = layer.get_underlying_layer_mut();
  blur::surface_blur(&mut *layer_ref, radius as u32, threshold as u8, None);
  layer.mark_dirty();
}
#[napi]
...
pub fn box_blur(layer: &mut Layer, radius: f64) {
  let layer_ref = layer.get_underlying_layer_mut();
  blur::box_blur(&mut *layer_ref, radius as f64, None);
  layer.mark_dirty();
}
...
pub fn lens_blur(layer: &mut Layer, options: Option<LensBlurOptions>) {
  ...
  blur::lens_blur(&mut *layer_ref, blur_options, None);
  layer.mark_dirty();
}
...
pub fn motion_blur(layer: &mut Layer, angle: f64, distance: f64) {
  let layer_ref = layer.get_underlying_layer_mut();
  blur::motion_blur(&mut *layer_ref, angle as f32, distance as u32, None);
  layer.mark_dirty();
}
