...
impl CanvasInner {
  ...
  pub fn new_from_path(
    ...
    canvas.set_canvas_size(width, height);
    canvas.layers[0].lock().unwrap().set_name("Background");
    canvas
  }
  ...
  pub fn add_adjustment_layer(
    &mut self, p_name: impl Into<String>, layer_type: crate::AdjustmentLayerType,
  ) -> Arc<Mutex<LayerInner>> {
    let name = p_name.into();
    let adjustment_layer = LayerInner::new_adjustment_layer(name, layer_type);
  ...
  pub fn delete_layer_by_id(&mut self, layer_id: &str) -> &mut Self {
    self.layers.retain(|layer_rc| layer_rc.lock().unwrap().id() != layer_id);
    self.mark_dirty();
    self
  }
