use crate::{Image, hsl_to_rgb, rgb_to_hsl};
use rayon::prelude::*;
...
pub fn blend_images_at(
  bottom_image: &mut Image, top_image: &Image, x1: i32, y1: i32, x2: i32, y2: i32, mode: fn(RGBA, RGBA) -> RGBA,
) {
...
pub fn blend_images_at_with_opacity(
  bottom_image: &mut Image, top_image: &Image, _x1: i32, _y1: i32, x2: i32, y2: i32, mode: fn(RGBA, RGBA) -> RGBA,
  opacity: f32,
) {
  ...
  pixels.par_chunks_mut(4).enumerate().for_each(|(i, chunk)| {
    ...
    let color = match (a, b) {
      (Some(a), Some(b)) => {
        ...
        ((out_r * 255.0) as u8, (out_g * 255.0) as u8, (out_b * 255.0) as u8, (out_a * 255.0) as u8)
      }
      (Some(a), None) => a,
      (None, Some(b)) => {
...
pub fn darker_color(a: RGBA, b: RGBA) -> RGBA {
  let lum_a = rgb_to_hsl(a.0, a.1, a.2).2;
  let lum_b = rgb_to_hsl(b.0, b.1, b.2).2;

  if lum_a < lum_b { a } else { b }
}

/// Selects the lighter of the two colors.
pub fn lighter_color(a: RGBA, b: RGBA) -> RGBA {
  let lum_a = rgb_to_hsl(a.0, a.1, a.2).2;
  let lum_b = rgb_to_hsl(b.0, b.1, b.2).2;
...
pub fn glow(a: RGBA, b: RGBA) -> RGBA {
  let glow_channel = |a: u8, b: u8| -> u8 {
    if a == 255 {
      255
    } else {
      ((b as f32 * b as f32) / (255.0 - a as f32)).min(255.0) as u8
    }
  };

  let red = glow_channel(a.0, b.0);
  let green = glow_channel(a.1, b.1);
  let blue = glow_channel(a.2, b.2);
  let alpha = glow_channel(a.3, b.3);
...
pub fn phoenix(a: RGBA, b: RGBA) -> RGBA {
  let phoenix_channel = |a: u8, b: u8| -> u8 {
    let a_f = a as f32;
    let b_f = b as f32;
    (a_f + b_f - 2.0 * a_f * b_f / 255.0).min(255.0).max(0.0) as u8
  };

  let red = phoenix_channel(a.0, b.0);
  let green = phoenix_channel(a.1, b.1);
  let blue = phoenix_channel(a.2, b.2);
  let alpha = a.3; // Preserve base layer alpha
...
pub fn negation(a: RGBA, b: RGBA) -> RGBA {
  let negation_channel = |a: u8, b: u8| -> u8 { (255.0 - (a as f32 - b as f32).abs()).min(255.0).max(0.0) as u8 };

  let red = negation_channel(a.0, b.0);
  let green = negation_channel(a.1, b.1);
  let blue = negation_channel(a.2, b.2);
  let alpha = a.3; // Preserve base layer alpha
...
pub fn grain_extract(a: RGBA, b: RGBA) -> RGBA {
  let grain_channel = |a: u8, b: u8| -> u8 { ((a as i32 + b as i32 - 255) / 2 + 128).clamp(0, 255) as u8 };

  let red = grain_channel(a.0, b.0);
  let green = grain_channel(a.1, b.1);
  let blue = grain_channel(a.2, b.2);
  let alpha = a.3; // Preserve base layer alpha
...
pub fn grain_merge(a: RGBA, b: RGBA) -> RGBA {
  let grain_channel = |a: u8, b: u8| -> u8 { ((a as i32 + b as i32 - 128) / 2).clamp(0, 255) as u8 };
...
pub fn soft_light(a: RGBA, b: RGBA) -> RGBA {
  ...
  let red = if b.0 < 128 {
    ...
    ((255.0 - 2.0 * (255.0 - a.0 as f32) * (255.0 - b.0 as f32) / 255.0) * blend_factor
      + a.0 as f32 * (1.0 - blend_factor)) as u8
  };
  let green = if b.1 < 128 {
    ((2.0 * a.1 as f32 * b.1 as f32 / 255.0) * blend_factor + a.1 as f32 * (1.0 - blend_factor)) as u8
  } else {
    ((255.0 - 2.0 * (255.0 - a.1 as f32) * (255.0 - b.1 as f32) / 255.0) * blend_factor
      + a.1 as f32 * (1.0 - blend_factor)) as u8
  };
  let blue = if b.2 < 128 {
    ((2.0 * a.2 as f32 * b.2 as f32 / 255.0) * blend_factor + a.2 as f32 * (1.0 - blend_factor)) as u8
  } else {
    ((255.0 - 2.0 * (255.0 - a.2 as f32) * (255.0 - b.2 as f32) / 255.0) * blend_factor
      + a.2 as f32 * (1.0 - blend_factor)) as u8
  };
  let alpha = if b.3 < 128 {
    ((2.0 * a.3 as f32 * b.3 as f32 / 255.0) * blend_factor + a.3 as f32 * (1.0 - blend_factor)) as u8
  } else {
    ((255.0 - 2.0 * (255.0 - a.3 as f32) * (255.0 - b.3 as f32) / 255.0) * blend_factor
      + a.3 as f32 * (1.0 - blend_factor)) as u8
  };
...
pub fn vivid_light(a: RGBA, b: RGBA) -> RGBA {
  let b_burn = ((2.0 * b.0 as f32) as u8, (2.0 * b.1 as f32) as u8, (2.0 * b.2 as f32) as u8, (2.0 * b.3 as f32) as u8);
  let b_dodge = (
...
pub fn reflect(a: RGBA, b: RGBA) -> RGBA {
  let reflect_channel = |a: u8, b: u8| -> u8 {
    if b == 255 {
      255
    } else {
      ((a as f32 * a as f32) / (255.0 - b as f32)).min(255.0) as u8
    }
  };

  let red = reflect_channel(a.0, b.0);
  let green = reflect_channel(a.1, b.1);
  let blue = reflect_channel(a.2, b.2);
  let alpha = reflect_channel(a.3, b.3);
...
pub fn blend_mode_name(mode: fn(RGBA, RGBA) -> RGBA) -> (&'static str, &'static str) {
  match () {
    _ if mode == normal => ("normal", "Normal"),
    _ if mode == darken => ("darken", "Darken"),
    _ if mode == darker_color => ("darker-color", "Darker Color"),
    _ if mode == average => ("average", "Average"),
    _ if mode == multiply => ("multiply", "Multiply"),
    _ if mode == color_burn => ("color-burn", "Color Burn"),
    _ if mode == linear_burn => ("linear-burn", "Linear Burn"),
    _ if mode == lighten => ("lighten", "Lighten"),
    _ if mode == lighter_color => ("lighter-color", "Lighter Color"),
    _ if mode == screen => ("screen", "Screen"),
    _ if mode == color_dodge => ("color-dodge", "Color Dodge"),
    _ if mode == linear_dodge => ("linear-dodge", "Linear Dodge"),
    _ if mode == overlay => ("overlay", "Overlay"),
    _ if mode == soft_light => ("soft-light", "Soft Light"),
    _ if mode == hard_light => ("hard-light", "Hard Light"),
    _ if mode == vivid_light => ("vivid-light", "Vivid Light"),
    _ if mode == linear_light => ("linear-light", "Linear Light"),
    _ if mode == pin_light => ("pin-light", "Pin Light"),
    _ if mode == hard_mix => ("hard-mix", "Hard Mix"),
    _ if mode == difference => ("difference", "Difference"),
    _ if mode == exclusion => ("exclusion", "Exclusion"),
    _ if mode == subtract => ("subtract", "Subtract"),
    _ if mode == divide => ("divide", "Divide"),
    _ if mode == hue => ("hue", "Hue"),
    _ if mode == saturation => ("saturation", "Saturation"),
    _ if mode == color => ("color", "Color"),
    _ if mode == luminosity => ("luminosity", "Luminosity"),
    _ if mode == reflect => ("reflect", "Reflect"),
    _ if mode == glow => ("glow", "Glow"),
    _ if mode == phoenix => ("phoenix", "Phoenix"),
    _ if mode == negation => ("negation", "Negation"),
    _ if mode == grain_extract => ("grain-extract", "Grain Extract"),
    _ if mode == grain_merge => ("grain-merge", "Grain Merge"),
    _ => ("unknown", "Unknown"),
  }
}
